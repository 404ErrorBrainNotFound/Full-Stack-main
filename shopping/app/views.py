...
def laptop(request, data=None):
    ...
    if data == 'Dell' or data == 'HP' or data == 'Lenovo':
        laptops = laptops.filter(brand=data)
    elif data == 'below':
        laptops = laptops.filter(discounted_price__lt=50000)
    elif data == 'above':
        laptops = laptops.filter(discounted_price__gte=100000)
...
class ProductView(View):
    def get(self, request):
        ...
        bottomwears = Product.objects.filter(category='BW')
        mobiles = Product.objects.filter(category='M')
        laptops = Product.objects.filter(category='Laptop')  # Add this line for laptops
        if request.user.is_authenticated:
            totalitem = len(Cart.objects.filter(user=request.user))
